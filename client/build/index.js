...
import { generateDeProof } from "./secure-proof.js";
dotenv.config();
...
class MCPClient {
        ...
        try {
            ...
            try {
                ...
                const timeoutPromise = new Promise((_, reject) => {
                    setTimeout(() => {
                        reject(new Error(`Connection to MCP server timeout (${MCP_CONNECTION_TIMEOUT_MS / 1000} seconds)`));
            ...
            console.log("Available tools:", this.tools.map((tool) => tool.function.name));
        }
        catch (e) {
    ...
    async processQuery(query) {
        ...
        try {
            ...
            const response = await this.openai.chat.completions.create(openaiRequestParams);
            ...
            if (toolCalls) {
                console.log("LLM requests tool call:", toolCalls.map((tc) => tc.function.name));
                ...
                try {
                    ...
                    for (const toolCall of toolCalls) {
                        ...
                        console.log(`Generating DeProof validation data for (cleaned) parameters:`, deProofGenArgs);
                        const deProof = await generateDeProof(deProofGenArgs, // Use cleaned args for DeProof generation
                        this.wallet, this.session);
                        // If it's a new session, update the client session state
                        if (!this.session) {
                            this.session = deProof.session;
                            console.log(`Created new session: ${this.session.substring(0, 8)}...`);
                        }
                        ...
                        const callToolWithTimeout = async () => {
                            ...
                            const timeoutPromise = new Promise((_resolve, reject) => {
                                setTimeout(() => {
                                    reject(new Error(`Tool call ${toolName} timed out after ${TOOL_CALL_TIMEOUT_MS / 1000} seconds`));
                        ...
                        try {
                            console.log(`Starting tool call: ${toolName}, using full DeProof validation`);
                            const result = (await callToolWithTimeout());
                            console.log(`Tool ${toolName} returned result:`, JSON.stringify(result.content).substring(0, 200) + "...");
                            messages.push({
                    ...
                    return `Error executing tool calls: ${error instanceof Error ? error.message : String(error)}`;
                }
                ...
                if (!secondResponse.choices ||
                    secondResponse.choices.length === 0 ||
                    !secondResponse.choices[0].message) {
                    console.error("LLM second response missing required fields", secondResponse);
